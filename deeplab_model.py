            ...
            with tf.contrib.slim.arg_scope(resnet_v2.resnet_arg_scope(batch_norm_decay=batch_norm_decay)):
                with arg_scope([layers.batch_norm], is_training=is_training):
                    ...
                    with tf.variable_scope("upsampling_logits"):
                        ...
                        net = layers_lib.conv2d(net, num_classes, [
...
def deeplabv3_plus_model_fn(features, labels, mode, params):
    ...
    confusion_matrix = tf.confusion_matrix(
        valid_labels, valid_preds, num_classes=params['num_classes'])
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        with tf.control_dependencies(update_ops):
            train_op = optimizer.minimize(
                loss, global_step, var_list=train_var_list)
    else:
        train_op = None
    ...
    mean_iou = tf.metrics.mean_iou(
        valid_labels, valid_preds, params['num_classes'])
